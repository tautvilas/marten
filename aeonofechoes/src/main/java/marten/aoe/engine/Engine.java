...
public final class Engine {
    ...
    public synchronized void switchMap (@NotNull String mapName, @NoNullEntries Player[] playerList) {
        if (MapLoader.getAvailableMaps().contains(mapName)) {
            this.map = MapLoader.loadMap(this, mapName);            
        } else if (MapLoader.getAvailableSimpleMaps().contains(mapName)) {
            this.map = MapLoader.loadSimpleMap(this, mapName);
    ...
    public synchronized TileDTO getTileDTO (@NotNull Player player, @NotNull PointDTO location) {
        this.validatePlayer(player);
    ...
    public synchronized UnitDTO getUnitDTO (@NotNull Player player, @NotNull PointDTO location) {
        this.validatePlayer(player);
        this.validateLocation(location);
    ...
    public synchronized void performAction (@NotNull Player player, @NotNull PointDTO from, @NotNull Action action, @NotNull PointDTO to) {
        this.validatePlayer(player);
    ...
    public synchronized void selectUnit (@NotNull Player player, @NotNull PointDTO location) {
        this.validatePlayer(player);
        this.validateLocation(location);
