...
public class ObjModelLoader implements GenericModelLoader {
	...
	public ComplexModel load(BufferedReader reader, String path) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			// use material
				...
				log.warn("Unrecognized obj file command '"
						+ command + "' on line " + lineNum);
			}
			lineNum++;
		}
	...
	private Triangle getTriangle(String[] params) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (vertexid < 0) vertexid = vertexes.size() - vertexid;
			else vertexid--;
			...
			if (!vertexParams[1].isEmpty()) {
				...
				if (texcoordid < 0) texcoordid = texcoords.size() - texcoordid;
				else texcoordid--;
			...
			if (!vertexParams[2].isEmpty()) {
				...
				if (normalid < 0) normalid = normals.size() - normalid;
				else normalid--;
	...
	private void checkLine(String[] params, int length) {
		if (params.length != length) {
			throw new RuntimeException("Invalid file format: "
					+ params.length + " fields instead of " + length + "on line " + lineNum);
		}
	}
	...
	private void parseMtlFile(String filename) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			//texture
				...
				checkLine(params, 1);
				texture = TextureLoader.loadTexture(path + params[0]);
			}
				...
				log.warn("Unrecognized mtl file command '"
						+ command + "' on line " + num);
			}
			num++;
		}
