...
import marten.age.geometry.TriangularGeometry;
...
import marten.age.util.TextureCoordinate;
...
public class ObjModelLoader implements GenericModelLoader {
    private static org.apache.log4j.Logger log = Logger
            .getLogger(ObjModelLoader.class);
    ...
    public ComplexModel load(BufferedReader reader, String path)
            throws Exception {
        this.path = path;
        ...
        while ((line = reader.readLine()) != null) {
            ...
            // face
                ...
                // in fact it could be faces of more vertexes but
                // we prefer triangles for performance reasons
                checkLine(params, 3);
            ...
            // comment and other useless? stuff
            ...
            // use material
                ...
                if (a == null) {
                    log.error("No such appearance " + params[0] + " ("
                            + lineNum + ")");
                } else {
                ...
                log.warn("Unrecognized obj file command '" + command
                        + "' on line " + lineNum);
            }
            lineNum++;
        }
    ...
    private Triangle getTriangle(String[] params) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (vertexid < 0)
                vertexid = vertexes.size() - vertexid;
            else
                vertexid--;
            ...
            if (!vertexParams[1].isEmpty()) {
                ...
                if (texcoordid < 0)
                    texcoordid = texcoords.size() - texcoordid;
                else
                    texcoordid--;
            ...
            if (!vertexParams[2].isEmpty()) {
                ...
                if (normalid < 0)
                    normalid = normals.size() - normalid;
                else
                    normalid--;
    ...
    private void checkLine(String[] params, int length) {
        if (params.length != length) {
            throw new RuntimeException("Invalid file format: " + params.length
                    + " fields instead of " + length + "on line " + lineNum);
        }
    }
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (command.equals("newmtl")) {
                ...
                if (!firstMaterial) {
                    ...
                    if (ambient != null)
                        app.setAmbient(ambient);
                    if (diffuse != null)
                        app.setDiffuse(diffuse);
                    if (specular != null)
                        app.setSpecular(specular);
                    if (texture != null)
            ...
            //texture
                ...
                texture = TextureLoader.loadTexture(
                        new ImageData(path + params[0]));
            }

            else if (command.isEmpty() || command.equals("#")
                    || command.equals("illum")) {
                // do nothing
            }

            else {
                log.warn("Unrecognized mtl file command '" + command
                        + "' on line " + num);
            }
            num++;
        }
